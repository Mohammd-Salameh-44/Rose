...
export default function Admin() {
  ...
  const [productSearchMode, setProductSearchMode] = useState("all");
  const [orderSearch, setOrderSearch] = useState("");
  const [orderSearchMode, setOrderSearchMode] = useState("all");
  ...
  const formatOrderDateTime = (order) => {
    const date = toDateFromOrder(order);
  ...
  const getProductSearchText = (product) => {
    ...
    if (productSearchMode === "name") return nameText;
    if (productSearchMode === "desc") return descText;

    return `${nameText} ${descText}`;
  };

  const getOrderSearchText = (order) => {
    const nameText = order.customerName || "";
    const phoneText = order.customerPhone || "";
    const addressText = order.customerAddress || "";
    const numberText = order.orderNumber || "";
    const dateText = formatOrderDateTime(order);

    if (orderSearchMode === "name") return nameText;
    if (orderSearchMode === "date") return dateText;
    if (orderSearchMode === "area") return addressText;
    if (orderSearchMode === "phone") return phoneText;

    return `${numberText} ${nameText} ${phoneText} ${addressText} ${dateText}`;
  };
  ...
  const filteredAdminProducts = (
    ...
    getProductSearchText(p).toLowerCase().includes(productSearch.toLowerCase())
  );

  const filteredOrders = orders.filter((order) =>
    getOrderSearchText(order).toLowerCase().includes(orderSearch.toLowerCase())
  );
    ...
    <div className="container">
      ...
      {adminPage === "dashboard" && (
        <>
          ...
          <div className="glass admin-box admin-search-panel">
            <div className="admin-search-row">
              <select
                value={productSearchMode}
                onChange={(e) => setProductSearchMode(e.target.value)}
              >
                <option value="all">{t.searchAll || "All"}</option>
                <option value="name">{t.searchByName || "Name"}</option>
                <option value="desc">{t.searchByDescription || "Description"}</option>
              </select>
        ...
        <>
          <div className="glass admin-box admin-search-panel">
            <div className="admin-search-row">
              ...
              >
                <option value="all">{t.searchAll || "All"}</option>
                <option value="name">{t.searchByCustomer || "Customer"}</option>
                <option value="date">{t.searchByDate || "Date"}</option>
                <option value="area">{t.searchByArea || "Area"}</option>
                <option value="phone">{t.searchByPhone || "Phone"}</option>
              </select>

              <input
                type="text"
                placeholder={t.searchOrders || t.searchCustomer}
                value={orderSearch}
            ...
            <div className="admin-products">
              {filteredOrders.map((order) => (
                <div key={order.id} className="glass admin-product-card">
                  ...
                  <p className="order-date-line">
                    {t.orderDate || "Order date"}: {formatOrderDateTime(order)}
                  </p>
